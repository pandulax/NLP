...
#closure_list = ['see', 'cordially', 'many', 'respectfully', 'cheers', 'wishes', 'with', 'my', 'truly', 'much', 'best', 'thanks', 'thanking', 'kind', 'appreciation', 'take', 'bests', 'regard', 'regards', 'all', 'so', 'sincere', 'sincerely', 'warm', 'ciao', 'faithfully', 'fond', 'thank', 'again', 'the', 'and', 'rgds', 'care', 'yours', 'warmly', 'around', 'gratitude', 'b', 'you', 'very', 'warmest']
#disclaimer_list = ['disclaimer', 'important', 'disseminate', 'contain', 'disclose', 'virus', 'liable', 'liability', 'confidential', 'privileged', 'email', 'e-mail','mail', 'warning', 'content', 'message', 'notice', 'transmit', 'individual', 'entity', 'immediate', 'distribute', 'prohibit', 'damage', 'authori', 'conclude', 'organization', 'attorney', 'protect']

closure_list = Closurevocab.closure_list
disclaimer_list = Disclaimervocab.disclaimer_list


def column_namer(self, prefix= '', val_list=[]):
    col_names =''
...
#----- proper_noun ----------------------------------------------------------------------
def proper_noun(self, string_line):
    count = 0
    
    string_line = string_line.replace('.', ' ')
    string_line = string_line.replace(',', ' ')
    
    str_list = string_line.split()
    
    for word in str_list:
        if [w.pos_ for w in nlp(word)][0] == 'PROPN':
            count += 1
...
def ngram_count(self, string_line ='', nglist = []):
    string_line = string_line.lower()
    string_line = string_line.replace(',', ' ')
    string_line = string_line.replace('.', ' ')
    string_line = string_line.replace(':', ' ')
...
def execute(csv_file_name = 'dataset.csv'):
    closure_cols = column_namer(0, 'C', closure_list)
    disclaimer_cols = column_namer(0, 'D', disclaimer_list)
    
    with open(csv_file_name, 'w') as outfile:
        #outfile.write('')
        outfile.write('class, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10,' + closure_cols + ',' + disclaimer_cols + '\n')
    
    with open('debug_'+csv_file_name, 'w') as outfile:
        #outfile.write('')
        outfile.write('fname, line, class, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10,' + closure_cols + ',' + disclaimer_cols + '\n')
    ...
    for labeled_f in labeled_list:
        ...
        for index in file_index:
            ...
            v10 = proper_noun(0, file_index[index][2])
            
            v11 = ngram_count(0, file_index[index][2], closure_list)
            
            v12 = ngram_count(0, file_index[index][2], disclaimer_list)
            
            #print(Y, str(v1)[:5], v2, v3, v4, v5, v6, v7, v8, v9, v10)
            
            dataset_arr =  [Y] + [v1] + [v2] + [v3] + [v4] + [v5] + [v6] + [v7] + [v8] + [v9] + [v10] + v11 + v12
            
            
            datasetcsv_string = ''
            for item_i in range( len(dataset_arr) ):
                if item_i == 0:
                    datasetcsv_string += str(dataset_arr[item_i])
                else:
                    datasetcsv_string += ',' + str(dataset_arr[item_i])
            
            with open('dataset.csv', 'a') as outfile:
                outfile.write(datasetcsv_string+'\n')
            ...
            debug_arr = [ file_index[index][0] ] + [index] + [Y] + [v1] + [v2] + [v3] + [v4] + [v5] + [v6] + [v7] + [v8] + [v9] + [v10] + v11 + v12
            
            fullcsv_string = ''
            for item_i in range( len(debug_arr) ):
                if item_i == 0:
                    fullcsv_string += str(debug_arr[item_i])
                else:
                    fullcsv_string += ',' + str(debug_arr[item_i])
            
            
            with open('debug_dataset.csv', 'a') as outfile:
                outfile.write(fullcsv_string+'\n')
